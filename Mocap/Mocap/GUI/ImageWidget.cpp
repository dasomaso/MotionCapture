...
ImageWidget::ImageWidget(QWidget *parent, int frameWidth, int frameHeight)
	: QGLWidget(QGLFormat(QGL::DoubleBuffer|QGL::IndirectRendering),parent)
{
	ui.setupUi(this);
	_imageRef = NULL;
	setAutoFillBackground(false);

	setMinimumSize(frameWidth, frameHeight);
	setMaximumSize(frameWidth, frameHeight);
}
...
void ImageWidget::paintEvent(QPaintEvent *paintEvent) {
	...
	//setupViewPort(width()/SCALE, height()/SCALE);
	setupViewPort(width(), height());
...
void ImageWidget::drawBackgroundImage() 
{
	...
	if (_imageRef != NULL) {
		...
		glDrawPixels(_imageRef->width, _imageRef->height, GL_LUMINANCE, GL_UNSIGNED_BYTE, _imageRef->imageData);
	}
}
