...
clear all;
clc;
%CreateCalibrationNeededDat;
calibrationPointsName = 'CalibrationPoints.txt';
camreaNum = CreateCalibrationData(calibrationPointsName);
% add necessary paths				
config = configdata(expname, camreaNum);
...
while selfcal.iterate & selfcal.count < config.cal.GLOBAL_ITER_MAX,
  % read the input data
  loaded = loaddata(config); %  load my own data
